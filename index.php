...
// คำนวณหา offset สำหรับคำสั่ง SQL
...
<div class="container">
  <div class="row">
    ...
    $query = mysqli_query($con, "SELECT * FROM ims_file ORDER BY id DESC LIMIT $per_page OFFSET $offset");
    while($result = mysqli_fetch_array($query)){
    ?>
      <div class="col-md-4 mb-4">
        <div class="card">
        <img class="card-img-top" src="PDO/upload/<?=$result['img']?>" alt="Card image cap" width="500" height="300">
          <div class="card-body">
            <h5 class="card-title"><?=$result['subject']?></h5>
            <p class="card-text"><?=$result['headline']?></p>
            <a href="./detail.php?id=<?=$result['id']?>" class="btn btn-primary">รายละเอียด</a>
          </div>
        </div>
      </div>
    <?php } ?>
    ...
    <div class="btn-toolbar text-center" role="toolbar" aria-label="Toolbar with button groups" style="display: block";>
        ...
        <?php
        // หาจำนวนข่าวทั้งหมด
        $total_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM ims_file"));
